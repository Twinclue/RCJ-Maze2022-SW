...
short move_robot::fwd(short remDist = 300){
    prePitch = imu->getFPitch();
    if(front->read(fc) < back->read(bc)){
        int startDist = front->read(fc);
        int errorDist = 0;
        imu->update();
        short startAng = imu->getFYaw();
        short errorAng = startAng - imu->getFYaw();
        fwdPid->init();
        while((errorDist < remDist) && (front->read(fc) > 50)){
            imu->update();
            errorAng = startAng - imu->getFYaw();
    ...
    right->on(0);
    this->corrDir();
    if(abs(prePitch - imu->getFPitch()) >= 15){
        return -2;
    }
    return 0;
}
...
short move_robot::turn(short remAng = 90){
    ...
    turnPid->init();
    if(remAng > 0){
        while(errorAng < remAng){//ここから
            imu->update();
            errorAng = imu->getFYaw() - startAng;
            left->on(turnPid->calcPI(errorAng,remAng));
            right->on(-turnPid->calcPI(errorAng,remAng));
            Serial.println(imu->getFYaw());//ここまで2ms
            delay(16);
        }
    }
    else{
...
short move_robot::goUp(){
    short prePitch = imu->getFPitch();
    while(abs(prePitch-imu->getFPitch()) <= 20){
        imu->update();
        Serial.println(prePitch - imu->getFPitch());
        left->on(255);
        right->on(255);
    }
    left->on(0);
